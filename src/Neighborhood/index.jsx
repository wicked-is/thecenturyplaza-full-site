...
import { WrapperStyled, HeaderStyled, ContainerStyled, ItemStyled, PairedStyled, ItemCaptionStyled, ItemHeadlineStyled, ItemCopyStyled, ItemCTAStyled } from 'Neighborhood/style.js';
...
const NeighborhoodItemPaired = styled.div`${PairedStyled};`;
...
const NeighborhoodItemCTA = styled.p`${ItemCTAStyled};`;
          ...
          {neighborhoodData.slice(1).map((item, index) => (
            <Grid.Unit key={index} size={{ phone: item.span[0] / 10, tabletLandscape: item.span[1] / 10 }}>
              <NeighborhoodItem valign={item.valign}>
                ...
                {item.cta.length > 0 && <NeighborhoodItemCTA><a href={item.url}>{item.cta}</a></NeighborhoodItemCTA>}
              </NeighborhoodItem>
            </Grid.Unit>
          ))}
        </Grid>
